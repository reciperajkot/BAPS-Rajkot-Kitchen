...
type Item = { id: string; name: string; meal_period: 'breakfast' | 'lunch' | 'dinner'; category_order?: number };
...
export default function BookingV2({ profileId, onSaved, s }: Props) {
  ...
  const [meal, setMeal] = useState<Item['meal_period']>('breakfast');
  const [date, setDate] = useState(''); const [name, setName] = useState(''); 
  const [mobile, setMobile] = useState(''); const [people, setPeople] = useState(''); 
  const [place, setPlace] = useState(''); const [receipt, setReceipt] = useState(''); 
  const [rasoi, setRasoi] = useState(''); const [thakorji, setThakorji] = useState(''); 
  const [time, setTime] = useState(times[0]); const [pickTime, setPickTime] = useState(false); 
  ...
  useEffect(() => { 
    ...
    // Fetch places
    supabase.from('booking_places').select('id,name').order('name').then(({ data }) => setPlaces((data || []) as Place[])); 
    
    // Fetch menu items WITH category_order and sort them by category_order FIRST, then by name
    supabase.from('available_menu_items')
      .select('id,name,meal_period,category_order')
      .order('category_order', { ascending: true }) // Category sorting (Mishthan first)
      .order('name', { ascending: true }) // Alphabetical sorting inside category
  ...
  async function save() {
    if (!date || !name.trim() || mobile.length !== 10 || !people || !place || !receipt.trim()) 
      return Alert.alert('Complete required details', 'Fill date, name, 10-digit mobile, people, place and receipt number.');
    
    if (!supabase) return; setSaving(true);
    
    const { error } = await supabase.rpc('create_booking', { 
      p_service_date: date, p_customer_name: name.trim(), p_mobile: mobile, p_people_count: Number(people), 
      p_place_id: place, p_service_time: time.value, p_receipt_number: receipt.trim(), 
      p_rasoi_seva_amount: Number(rasoi) || 0, p_thakorji_seva_amount: Number(thakorji) || 0, 
      p_menu_item_ids: selected, p_custom_values: {} 
    });
    
    setSaving(false); 
    if (error) return Alert.alert('Could not save', error.message); 
    Alert.alert('Booking saved', `Receipt ${receipt} has been saved.`); 
    onSaved();
  }

  return <View>
    <Text style={s.h1}>New Booking / નવું બુકિંગ</Text>
    ...
    <Text style={s.label}>Menu / મેનુ</Text>
    <View style={s.roleRow}>{(['breakfast','lunch','dinner'] as const).map(m=><Pressable key={m} onPress={()=>setMeal(m)} style={[s.roleChip,meal===m&&s.roleChipOn]}><Text>{m[0].toUpperCase()+m.slice(1)}</Text></Pressable>)}</View>
    ...
    <View style={s.booking}><Text style={s.userName}>Final Total / અંતિમ કુલ</Text><Text style={s.value}>₹{finalTotal.toFixed(2)}</Text></View>
    <Pressable disabled={saving} onPress={save} style={[s.primary,saving&&s.disabled]}><Text style={s.primaryText}>{saving?'Saving…':'Save Booking / બુકિંગ સેવ કરો'}</Text></Pressable>
