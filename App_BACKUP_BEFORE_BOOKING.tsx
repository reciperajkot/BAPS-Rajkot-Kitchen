...
function Dashboard({ profile }: { profile: Profile }) {
  const [bookings, setBookings] = useState<Booking[]>([]); const [loadingBookings, setLoadingBookings] = useState(true); const [screen, setScreen] = useState<'home' | 'new' | 'bookings'>('home');
  ...
  const page = screen === 'new' ? <BookingV2 profileId={profile.id} s={s} onSaved={() => { setScreen('bookings'); loadBookings(); }} /> : screen === 'bookings' ? <BookingList bookings={bookings} loading={loadingBookings} title="Bookings" /> : <Home profile={profile} bookings={bookings} loading={loadingBookings} onNew={() => setScreen('new')} onList={() => setScreen('bookings')} />;
