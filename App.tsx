  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      ...
      if (error || !data || !data.is_active) {
        showMsg('ભૂલ', 'તમારું એકાઉન્ટ બંધ છે અથવા પ્રોફાઇલ મળતી નથી.');
      ...
      if (data.duty_places) {
          ...
          else if (typeof data.duty_places === 'string') { try { parsedPlaces = JSON.parse(data.duty_places); } catch(e) { parsedPlaces = [data.duty_places]; } }
      } else if ((data as any).duty_place) { parsedPlaces = [(data as any).duty_place]; }
...
function LoginScreen() {
  ...
  async function signIn() {
    ...
    setSubmitting(false);
    if (error) showMsg('લૉગિન નિષ્ફળ', 'યુઝર ID અથવા પાસવર્ડ ખોટો છે.');
  }
    ...
    <SafeAreaView style={s.loginSafe}>
      <View style={s.webContainer}>
        <View style={s.loginCard}>
          ...
          <TextInput value={password} onChangeText={setPassword} placeholder="પાસવર્ડ" placeholderTextColor="#9ca3af" style={s.input} secureTextEntry onSubmitEditing={signIn} />
...
function AdminHome({ session, profile }: { session: Session, profile: Profile }) { 
  ...
  async function fetchDashboard() {
    ...
    try {
      ...
      if (data) {
        ...
        let tData = { total: 0, mealMap: {} as any };
        let tomData = { total: 0, mealMap: {} as any };
        ...
        data.forEach(b => {
          ...
          if (b.meals) {
            b.meals.forEach((m: any) => {
              ...
              const hostName = `${b.name || ''} ${b.surname || ''}`.trim() || 'અજ્ઞાત યજમાન';
              
              if (m.date === tStr) {
                tData.total += g;
                if(!tData.mealMap[m.mainType]) tData.mealMap[m.mainType] = { count: 0, hosts: [] };
                tData.mealMap[m.mainType].count += g;
                tData.mealMap[m.mainType].hosts.push(`${hostName} (${g} લોકો)`);
              }
              if (m.date === tomStr) {
                tomData.total += g;
                if(!tomData.mealMap[m.mainType]) tomData.mealMap[m.mainType] = { count: 0, hosts: [] };
                tomData.mealMap[m.mainType].count += g;
                tomData.mealMap[m.mainType].hosts.push(`${hostName} (${g} લોકો)`);
              }
            });
          }
        });
  ...
  return (
    <View style={s.p18}>
      ...
      <PreviewWidget title="👥 આજના યજમાનો" dateStr={getTodayStr()} data={todayData} bgColor="#f0fdf4" borderColor="#6ee7b7" />
      <PreviewWidget title="👥 આવતીકાલના યજમાનો" dateStr={getTomorrowStr()} data={tomorrowData} bgColor="#fffbeb" borderColor="#fde047" />
...
// Reusable Preview Widget for Today / Tomorrow
function PreviewWidget({ title, dateStr, data, bgColor, borderColor }: any) {
  return (
    <View style={[s.todayGuestsCard, { backgroundColor: bgColor, borderColor: borderColor }]}>
        ...
        {Object.keys(data.mealMap).length === 0 ? (
          <Text style={{color: '#64748b', fontSize: 14, marginTop: 4, paddingBottom: 10}}>કોઈ જમણવાર નોંધાયેલ નથી.</Text>
        ) : (
          <View style={{marginTop: 6}}>
            {Object.keys(data.mealMap).map(mType => (
              <View key={mType} style={{marginBottom: 12}}>
                ...
                <View style={{paddingLeft: 10, marginTop: 6}}>
                   {data.mealMap[mType].hosts.map((h:string, idx:number) => (
                      <Text key={idx} style={{color: '#334155', fontSize: 13, marginBottom: 3}}>• {h}</Text>
                   ))}
                </View>
              </View>
            ))}
            <View style={{borderTopWidth: 1, borderTopColor: borderColor, paddingTop: 8, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
...
// ================= ADMIN MENU SCREEN (WITH SORTING) =================
function AdminMenuScreen({ onBack }: { onBack: () => void }) {
  const [name, setName] = useState(''); const [mainType, setMainType] = useState('લંચ'); const [subCategory, setSubCategory] = useState('રોટલી'); const [price, setPrice] = useState(''); 
  const [menuItems, setMenuItems] = useState<any[]>([]); const [editingId, setEditingId] = useState<string | null>(null);
  const [dynamicMainTypes, setDynamicMainTypes] = useState<string[]>(BASE_MAIN_TYPES); const [dynamicSubTypes, setDynamicSubTypes] = useState<string[]>(BASE_SUB_TYPES);
  ...
  async function fetchMenu() {
    ...
    if (data) {
      setMenuItems(data);
      // Sort main and sub types based on their DB sort values
      ...
      if (!finalMains.includes(mainType)) setMainType(finalMains[0] || 'લંચ');
      if (!finalSubs.includes(subCategory)) setSubCategory('રોટલી');
    }
  }

  async function saveMenu() {
    if (!name.trim() || !price || !supabase) return showMsg('Error', 'વાનગીનું નામ અને સેવા (ભાવ) લખો.');
    const finalSub = subCategory || 'રોટલી';
    const mainSortVal = dynamicMainTypes.indexOf(mainType);
    const subSortVal = dynamicSubTypes.indexOf(finalSub);
    const payload = { name: name.trim(), main_type: mainType, sub_category: finalSub, price: parseFloat(price) || 0, is_active: true, main_sort: mainSortVal > -1 ? mainSortVal : 99, sub_sort: subSortVal > -1 ? subSortVal : 99 };
    try {
  ...
  // Sorting Logic (Expert implementation without new tables)
  async function moveCategory(type: 'main'|'sub', index: number, direction: -1 | 1) {
    const list = type === 'main' ? [...dynamicMainTypes] : [...dynamicSubTypes];
    ...
    if (type === 'main') setDynamicMainTypes(list); else setDynamicSubTypes(list);
    
    // Update DB in background
    if (supabase) {
       try {
         const updates = list.map((catName, idx) => {
            return type === 'main' 
               ? supabase.from('menu_items').update({main_sort: idx}).eq('main_type', catName)
               : supabase.from('menu_items').update({sub_sort: idx}).eq('sub_category', catName);
         });
  ...
  async function deleteMenu(id: string) {
    ...
    Alert.alert('કન્ફર્મ કરો', 'આ વાનગી કાઢી નાખવી છે?', [ { text: 'ના'}, { text: 'હા, કાઢો', style: 'destructive', onPress: async () => { await supabase!.from('menu_items').delete().eq('id', id); fetchMenu(); }}]);
  }

  async function savePrompt() {
    if(!promptData.value.trim() || !supabase) { setPromptData({...promptData, visible:false}); return; }
    const newVal = promptData.value.trim();
    try {
      if(promptData.type === 'main' && promptData.oldValue) await supabase.from('menu_items').update({main_type: newVal}).eq('main_type', promptData.oldValue);
      else if (promptData.type === 'sub' && promptData.oldValue) await supabase.from('menu_items').update({sub_category: newVal}).eq('sub_category', promptData.oldValue);
      else if (promptData.type === 'addMain') setMainType(newVal);
      else if (promptData.type === 'addSub') setSubCategory(newVal);
      fetchMenu();
    } catch(err:any) { showMsg('Error', err.message); }
  ...
  return (
    <View style={s.p18}>
      ...
      <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15, flexWrap:'wrap', gap:10}}>
        ...
        <Pressable onPress={()=>setShowCatManager(true)} style={[s.refreshBtn, {backgroundColor: '#1e293b'}]}><Text style={[s.refreshBtnText, {color: '#fff'}]}>⚙️ કેટેગરી અને ક્રમ (Sort) સેટિંગ્સ</Text></Pressable>
      </View>
      ...
      <Modal visible={showCatManager} animationType="slide">
        <SafeAreaView style={s.safe}>
           <View style={[s.header, {paddingTop: Platform.OS==='web'?20:0}]}><Text style={s.h1}>કેટેગરી અને ક્રમ મેનેજમેન્ટ</Text><Pressable onPress={()=>setShowCatManager(false)}><Text style={{fontSize:24, color:'#64748b'}}>✕</Text></Pressable></View>
           <ScrollView contentContainerStyle={[s.webContainer, s.p18]}>
              <View style={s.formCard}>
                <Text style={s.sectionTitle}>જમણવાર ના પ્રકાર (ઉપર/નીચે કરવા એરો દબાવો)</Text>
                {dynamicMainTypes.map((t, idx) => (
                  <View key={t} style={{flexDirection:'row', justifyContent:'space-between', paddingVertical:12, borderBottomWidth:1, borderColor:'#eee', alignItems:'center'}}>
                    <Text style={{fontSize:16, fontWeight:'bold', color:'#1e293b'}}>{idx + 1}. {t}</Text>
                    <View style={{flexDirection:'row', gap:8, alignItems: 'center'}}>
                       <Pressable onPress={()=>moveCategory('main', idx, -1)} style={s.editBtn}><Text>▲</Text></Pressable>
                       <Pressable onPress={()=>moveCategory('main', idx, 1)} style={s.editBtn}><Text>▼</Text></Pressable>
                       <Pressable onPress={()=>setPromptData({visible:true, title:'જમણવાર એડિટ કરો', type:'main', oldValue:t, value:t})} style={s.editBtn}><Text style={s.editBtnText}>✏️</Text></Pressable>
                    </View>
                  </View>
                ))}
              </View>
              <View style={[s.formCard, {marginTop: 20}]}>
                ...
                {dynamicSubTypes.map((t, idx) => (
                  <View key={t} style={{flexDirection:'row', justifyContent:'space-between', paddingVertical:12, borderBottomWidth:1, borderColor:'#eee', alignItems:'center'}}>
                    <Text style={{fontSize:16, fontWeight:'bold', color:'#1e293b'}}>{idx + 1}. {t}</Text>
                    <View style={{flexDirection:'row', gap:8, alignItems: 'center'}}>
                       <Pressable onPress={()=>moveCategory('sub', idx, -1)} style={s.editBtn}><Text>▲</Text></Pressable>
                       <Pressable onPress={()=>moveCategory('sub', idx, 1)} style={s.editBtn}><Text>▼</Text></Pressable>
                       <Pressable onPress={()=>setPromptData({visible:true, title:'વાનગીનો પ્રકાર એડિટ કરો', type:'sub', oldValue:t, value:t})} style={s.editBtn}><Text style={s.editBtnText}>✏️</Text></Pressable>
                    </View>
                  </View>
                ))}
              </View>
      ...
      <Modal visible={promptData.visible} transparent animationType="fade">
        <View style={s.modalBg}>
          <View style={s.modalContent}>
            ...
            <TextInput style={s.input} value={promptData.value} onChangeText={(v)=>setPromptData({...promptData, value:v})} onSubmitEditing={savePrompt} autoFocus />
            <Pressable onPress={savePrompt} style={s.primary}><Text style={s.primaryText}>સેવ કરો</Text></Pressable>
...
function AdminPlacesScreen({ onBack }: { onBack: () => void }) {
  ...
  return (
    <View style={s.p18}>
      ...
      <View style={s.formCard}>
        <TextInput style={s.input} value={name} onChangeText={setName} placeholder="સ્થળનું નામ" onSubmitEditing={savePlace} />
        <Pressable onPress={savePlace} style={s.primary}><Text style={s.primaryText}>સેવ કરો</Text></Pressable>
      </View>
      {places.map(p => (
...
function AdminUsersScreen({ onBack }: { onBack: () => void }) {
  ...
  const [showForm, setShowForm] = useState(false); const [editingId, setEditingId] = useState<string | null>(null); const [saving, setSaving] = useState(false);
  const [name, setName] = useState(''); const [mobile, setMobile] = useState(''); const [role, setRole] = useState<Role>('counter');
  const [selectedDutyPlaces, setSelectedDutyPlaces] = useState<string[]>([]); const [loginEmail, setLoginEmail] = useState(''); const [loginPass, setLoginPass] = useState('');
  ...
  async function saveUser() {
    ...
    if (role === 'counter' && selectedDutyPlaces.length === 0) return showMsg('ભૂલ ❌', 'કાઉન્ટર માટે એક સ્થળ પસંદ કરો.');
    ...
    if (editingId) {
      ...
      setSaving(false);
      if(error) showMsg('ભૂલ ❌', error.message); else { showMsg('સફળતા 🎉', 'યુઝર અપડેટ થઈ ગયો!'); setShowForm(false); setEditingId(null); fetchData(); }
    } else {
      try {
        const { data } = await supabase.auth.getSession(); const token = data?.session?.access_token;
        if (!token) { setSaving(false); return showMsg('સેશન એક્સપાયર', 'ફરીથી લૉગિન કરો.'); }
        const res = await fetch('https://ooeecqioprwverlpdjqe.supabase.co/functions/v1/create-auth-user', {
          method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ email: authEmail, password: loginPass, full_name: name, mobile, role })
        });
    ...
    <View style={s.p18}>
      ...
      {showForm ? (
        <View style={s.formCard}>
          ...
          <Text style={s.label}>માન્ય સ્થળ (એકથી વધુ સિલેક્ટ કરી શકાય)</Text>
          <View style={s.chipContainer}>
             {places.map(p => {
                 const isSelected = selectedDutyPlaces.includes(p.id);
                 return <Pressable key={p.id} onPress={() => setSelectedDutyPlaces(isSelected ? selectedDutyPlaces.filter(id => id !== p.id) : [...selectedDutyPlaces, p.id])} style={[s.chip, isSelected && s.chipSelected]}><Text style={[s.chipText, isSelected && s.chipTextSelected]}>{p.name}</Text></Pressable>
             })}
          </View>
          ...
          <Pressable disabled={saving} onPress={saveUser} style={[s.primary, saving && {opacity: 0.7}]}><Text style={s.primaryText}>{saving ? 'સેવ થઈ રહ્યું છે...' : 'સેવ કરો'}</Text></Pressable>
          <Pressable onPress={() => {setShowForm(false); setEditingId(null);}} style={s.closeButton}><Text>કેન્સલ</Text></Pressable>
        </View>
...
function CounterHome({ session, profile }: { session: Session, profile: Profile }) {
  ...
  return (
    <View style={s.p18}>
      ...
      <PreviewWidget title="🍽️ આજના યજમાનો" dateStr={getTodayStr()} data={todayData} bgColor="#f0fdf4" borderColor="#6ee7b7" />
      <PreviewWidget title="🍽️ આવતીકાલના યજમાનો" dateStr={getTomorrowStr()} data={tomorrowData} bgColor="#fffbeb" borderColor="#fde047" />
      ...
      <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8}}>
        ...
        <Pressable onPress={fetchDashboard} style={s.refreshBtn}><Text style={s.refreshBtnText}>🔄 રિફ્રેશ</Text></Pressable>
      </View>
      {todaysMeals.map(b => <BookingCard key={b.id} b={b} places={places} isAdmin={false} onEdit={setEditingBooking} /> )}
      <Pressable onPress={() => setActiveTab('all_bookings')} style={[s.primary, {backgroundColor: '#ffffff', borderWidth: 2, borderColor:'#047857', borderStyle:'dashed', marginTop: 12}]}><Text style={{color:'#047857', fontWeight:'bold', fontSize: 16}}>📋 બધા બુકિંગ્સ જુઓ (View All)</Text></Pressable>
    </View>
  );
}
...
function BookingScreen({ onBack, session, profile, initialData, allowedPlaces }: { onBack: () => void, session: Session, profile: Profile, initialData?: any, allowedPlaces?: any[] }) {
  const [places, setPlaces] = useState<any[]>(allowedPlaces || []); const [menuItems, setMenuItems] = useState<any[]>([]); const [saving, setSaving] = useState(false);
  const [name, setName] = useState(initialData?.name || ''); const [father, setFather] = useState(initialData?.father || ''); const [surname, setSurname] = useState(initialData?.surname || ''); const [mobile, setMobile] = useState(initialData?.mobile || '');
  
  let defaultPlace = initialData?.place_id || null;
  if (!defaultPlace && profile.role === 'counter' && allowedPlaces && allowedPlaces.length === 1) defaultPlace = allowedPlaces[0].id;
  const [selectedPlaceId, setSelectedPlaceId] = useState<string | null>(defaultPlace);
  
  const [meals, setMeals] = useState<any[]>(initialData?.meals || []); const [showMealBuilder, setShowMealBuilder] = useState(false); const [editingMealIndex, setEditingMealIndex] = useState<number | null>(null);
  const [thakorjiSeva, setThakorjiSeva] = useState(initialData?.thakorji_seva?.toString() || '0'); const [receiptNo, setReceiptNo] = useState(initialData?.receipt_no || '');
  const [paymentStatus, setPaymentStatus] = useState(initialData?.payment_status || 'પૂર્ણ સેવા'); 

  useEffect(() => {
    async function fetchData() {
      if (!supabase) return;
      if (!allowedPlaces) { const { data } = await supabase.from('places').select('*').eq('is_active', true); if (data) setPlaces(data.map(p => ({ label: p.name, value: p.id }))); } 
  ...
  const grandTotal = mealsTotal + (parseInt(thakorjiSeva) || 0); const diffTotal = grandTotal - (initialData ? (initialData.grand_total || 0) : 0);
  ...
  async function handleSaveBooking() {
    ...
    if (!supabase) return; setSaving(true);
    try {
      const payload = { name, father, surname, mobile, place_id: selectedPlaceId, meals, rasoi_seva: 0, thakorji_seva: (parseInt(thakorjiSeva) || 0), receipt_no: receiptNo, payment_status: paymentStatus, grand_total: grandTotal, user_id: session.user.id };
  ...
  return (
    <View style={s.p18}>
      ...
      <View style={s.formCard}>
        ...
        <Pressable disabled={saving} onPress={handleSaveBooking} style={s.saveBtn}><Text style={s.saveBtnText}>{saving ? 'Saving...' : 'બુકિંગ ફાઇનલ સેવ કરો'}</Text></Pressable>
      </View>
...
// ================= PRODUCTION (રસોડું) MODULE (WITH DATE TOGGLE) =================
function ProductionHome() {
  const [loading, setLoading] = useState(true);
  const [prodDate, setProdDate] = useState<'today'|'tomorrow'>('today'); // New Toggle
  const [menuAggregates, setMenuAggregates] = useState<any>({});
  const [totalGuests, setTotalGuests] = useState(0);
  const [dispatchSchedule, setDispatchSchedule] = useState<any[]>([]);
  ...
  async function fetchProductionData() {
    ...
    try {
      ...
      if (error) throw error;
      if (data) {
        let agg: any = {}; let tSchedule: any[] = []; let tGuests = 0;
        data.forEach(b => {
          let placeName = pData?.find((p:any) => p.id === b.place_id)?.name || 'અન્ય સ્થળ';
          if (!agg[placeName]) agg[placeName] = {};
          if (b.meals) {
            b.meals.forEach((m: any) => {
              if (m.date === targetDateStr) {  // Match with selected date!
                ...
                tSchedule.push({ hostName: `${b.name}`, time: m.time, timeValue: parseTimeForSort(m.time), placeName: placeName, mainType: m.mainType, guestsCount: gCount, items: m.items?.map((i:any)=>i.name) || [], note: m.note });
              }
            });
          }
        });
        tSchedule.sort((a, b) => a.timeValue - b.timeValue);
        setMenuAggregates(agg); setTotalGuests(tGuests); setDispatchSchedule(tSchedule);
      }
    } catch (err: any) { showMsg('Error', err.message); } finally { setLoading(false); }
  }
        ...
        <View style={s.modalBg}>
          <View style={s.modalContent}>
            <ScrollView>
               ...
               {onAddNew && ( <Pressable style={s.modalItem} onPress={() => { setVisible(false); onAddNew(); }}><Text style={{color: '#047857', fontWeight: '900'}}>＋ નવો પ્રકાર ઉમેરો</Text></Pressable> )}
            </ScrollView>
...
function MealBuilderModal({ visible, onClose, onSave, menuItems, initialData }: any) {
  ...
  useEffect(() => {
    if (visible) {
      if (initialData) { setDate(initialData.date); setTime(initialData.time); setGuestsCount(initialData.guestsCount.toString()); setMainType(initialData.mainType); setCalculatedTotal(initialData.ratePerPlate); const m: any = {}; initialData.items.forEach((i:any) => m[i.id] = true); setSelectedItems(m); } 
...
const s = StyleSheet.create({
  ...
  loginCard:{backgroundColor:'#ffffff',borderRadius:20,padding:30, borderWidth: 1, borderColor: '#e2e8f0'}, 
  ...
  todayGuestsCard: { borderRadius: 16, padding: 18, borderWidth: 1.5, marginVertical: 10 },
  ...
  bookingCard: { backgroundColor: '#ffffff', borderRadius: 16, padding: 16, marginBottom: 14, borderWidth: 1, borderColor: '#e2e8f0' },
